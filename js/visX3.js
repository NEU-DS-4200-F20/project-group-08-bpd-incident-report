...
((() => {
 ...
 var pie = d3.pie() // start and end angles of the segments
  ...
  // define tooltip
  var tooltip = d3.select('#visX2pc') // select element in the DOM with id 'chart'
...
path.on('mouseover', function(d) {  // when mouse enters div      
 ...
 tooltip.select('.action').html(d.action); // set current label           
...
// define legend
var legend = svg.selectAll('.visX2pc') // selecting elements with class 'legend'
...
legend.append('rect') // append rectangle squares to legend                                   
  ...
  .on('click', function(action) {
    ...
    pie.value(function(d) { 
      if (d.action === action) d.enabled = enabled; // if entry label matches legend label
