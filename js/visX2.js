...
((() => {
  ...
  d3.csv("./data/pie_cc_find.csv", function(d) {
  	return {
      finding : d.finding,
      count : +d.count,
    };
...
// gets the count for all the values being shown
 var pie = d3.pie() 
  ...
  .append('path') 
  .attr('d', arc) 
  .attr('fill', function(d) { return color(d.data.finding); }) 
...
path.on('mouseover', function(d) {  
 ...
 tooltip.select('.finding').html(nd.finding);         
  ...
  .on('click', function(e) {
   var finding = e.target.__data__;
    ...
    pie.value(function(d) { 
      if (d.finding === finding) d.enabled = enabled;  
        return (d.enabled) ? d.count : 0;  
    });
